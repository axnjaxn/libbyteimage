...
ByteImage ByteImage::odd() const {
  ByteImage img(nr >> 1, nc, nchannels);

  for (int ch = 0; ch < img.nchannels; ch++) 
    for (int r = 0; r < img.nr; r++)
      memcpy(img.pixels + ch * img.nr * img.nc + r * img.nc,
	     pixels + ch * nr * nc + ((r << 1) | 1) * nc, 
	     nc);
  
  return img;
}

ByteImage ByteImage::even() const {
  ByteImage img((nr >> 1) + (nr & 1), nc, nchannels);

  for (int ch = 0; ch < img.nchannels; ch++) 
    for (int r = 0; r < img.nr; r++)
      memcpy(img.pixels + ch * img.nr * img.nc + r * img.nc,
	     pixels + ch * nr * nc + (r << 1) * nc, 
	     nc);
  
  return img;
}

ByteImage ByteImage::oddAvg() const {
  ByteImage img(nr, nc, nchannels);

  for (int ch = 0; ch < img.nchannels; ch++) 
    for (int r = 1; r < img.nr; r += 2)
      memcpy(img.pixels + ch * img.nr * img.nc + r * img.nc,
	     pixels + ch * nr * nc + r * nc, 
	     nc);

  for (int ch = 0; ch < img.nchannels; ch++)
    memcpy(img.pixels + ch * img.nr * img.nc, img.pixels + ch * img.nr * img.nc + img.nc, nc);

  if (nr & 1)
    for (int ch = 0; ch < img.nchannels; ch++)
      memcpy(img.pixels + ch * img.nr * img.nc + (nr - 1) * img.nc, img.pixels + ch * img.nr * img.nc + (nr - 2) * img.nc, nc);

  for (int ch = 0; ch < img.nchannels; ch++)
    for (int r = 2; r < img.nr; r += 2)
      for (int c = 0; c < img.nc; c++)
	img.at(r, c, ch) = avg(img.at(r - 1, c, ch), img.at(r + 1, c, ch));
  
  return img;
}

ByteImage ByteImage::evenAvg() const {
  ByteImage img(nr, nc, nchannels);
...
ByteImage ByteImage::odd_d2() const {
  return odd().dropCols();  
}

ByteImage ByteImage::even_d2() const {
  return even().dropCols();
}

ByteImage ByteImage::dropCols() const {
  ByteImage result(nr, nc >> 1, nchannels);

  ByteImage::BYTE *spx, *dpx;
  for (int ch = 0; ch < nchannels; ch++)
    for (int r = 0; r < nr; r++) {
      spx = pixels + ch * nr * nc + r * nc;
      dpx = result.pixels + ch * result.nr * result.nc + r * result.nc;
      for (int c = 0; c < result.nc; c++) {
	*dpx = *spx;
	dpx++;
        spx += 2;
...
ByteImage ByteImage::expandRows() const {
  ByteImage img((nr << 1) - 1, nc, nchannels);
